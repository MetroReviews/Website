        ...
        <>
          {banner && <div className="bg-amber-900/20 py-2 px-4 rounded-xl mt-5 md:flex items-center md:justify-between">
              ...
              <div className="flex items-center justify-end md:justify-start space-x-4">
                <a href="/github" target="_blank" className="hover:underline text-white font-light">
                    ...
                    <div className="flex items-center space-x-6">
                        <div className="flex items-center space-x-3">
                            <img src="/img/logo.webp" 
                                ...
                                width="48" height="48"
                            />
                            <p className="invisible md:visible text-xl text-white font-semibold" dangerouslySetInnerHTML={{ __html: $.title }} />
                        </div>
                        <ul className="hidden lg:flex items-center space-x-4">
                            {NavItems.filter(a => a.link).map((item, itemIndex) => (
                                <li key={itemIndex}>
                                    <Link href={item.href} >
                                        <a className={`border-b-2 ${router.asPath === item.href ? 'text-white hover:text-amber-500 border-amber-500' : 'border-black/0 text-white hover:text-amber-500'} transition-all duration-200 font-medium pb-3`}>
                                            {item.name}
                                        </a>
                                    </Link>
                                </li>
                            ...
                            {NavItems.filter(a => !a.link).map((item, itemIndex) => (
                                <li key={itemIndex}>
                                    <a target="_blank" href={item.href} >
                                        <div className={`border-b-2 ${router.asPath === item.href ? 'text-white hover:text-amber-500 border-amber-500' : 'border-black/0 text-white hover:text-amber-500'} transition-all duration-200 font-medium`}>
                                            {item.name}
                                        </div>
                                    </a>
                                </li>
